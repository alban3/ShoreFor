...
def reading_files(file_forcing, file_shoreline, init_c, init_f):
    ...
    forcing_ncFile = Dataset(file_forcing)
    hs = forcing_ncFile.variables['hs'][:]
    tpf = forcing_ncFile.variables['tp'][:]
    timef = forcing_ncFile.variables['date'][:]
    forcing_ncFile.close()
    ...
    return (times, time_shor, loc_init, loc_extra,
            timef, deltat, ii0, iif, vecind, same, data, data_std, frag,
